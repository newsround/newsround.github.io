...
function setupEventListeners() {
    ...
    elements.viewBtns.forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            elements.newsContainer.classList.toggle('list-view', btn.dataset.view === 'list');
        });
    });
...
function renderFeaturedNews(articles) {
    ...
    if (articles.length > 0) {
        ...
        html += `
            <div class="featured-main" onclick="window.open('${mainArticle.url}', '_blank')">
        ...
        for (let i = 1; i < Math.min(articles.length, 3); i++) {
            ...
            html += `
                <div class="featured-secondary" onclick="window.open('${article.url}', '_blank')">
                    <div class="news-category">${currentCategory.toUpperCase()}</div>
...
function renderNews(articles) {
    ...
    const newsHtml = articles.map(article => {
        const imageUrl = article.urlToImage || 'https://images.unsplash.com/photo-1588681664899-f142ff2dc9b1?ixlib=rb-4.0.3&auto=format&fit=crop&w=800&q=80';
        ...
        return `
            <div class="news-card" onclick="window.open('${article.url}', '_blank')">
                ...
                <div class="news-content">
                    ...
                    <p class="news-description">${article.description || '暂无描述'}</p>
                    <div class="news-meta">
